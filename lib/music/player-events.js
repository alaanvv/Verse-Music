...
module.exports = bot => {
  bot.manager
    ...
    .on('playerDestroy', async (player) => {
      if (player.textChannel && player.guild) bot.chat.updateBotChat(player, true)
    })
    .on('trackStart', async (player, track) => {
      ...
      bot.chat.updateBotChat(player)
      player.set('previoustrack', track)
      player.set('pause', false)
    })
    .on('trackEnd', async (player, track, payload) => {
      unlink(process.cwd() + '/img/song.png', err => {throw err})
    })
    .on('trackStuck', async (player, track, payload) => {
      await player.stop()
      if (player.textChannel) bot.chat.updateBotChat(player)
    })
