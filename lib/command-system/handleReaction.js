...
module.exports = bot => {
  bot.on('messageReactionAdd', async (reaction, user) => {
    const botEmbedID = bot.chat.getEmbed(reaction.message.guild.id)
    ...
    switch (bot.reactions[reaction.emoji]) {
      ...
      case 'shuffle':
        ...
        break
      case 'save':
        // Save need to be rewrited because its different when on reaction
        const player = bot.manager.players.get(reaction.message.guildId)

        const member = await bot.utils.search(reaction.message.channel.guild.members, m => m.id == user.id)

        if (!player && member.voice.channel || player && !player.queue.current && member.voice.channelId === player?.voiceChannel) { // ADD TO QUEUE
          ...
          for (const track of pl) {
            const unresolvedTrack = TrackUtils.buildUnresolved({ title: track.title, url: track.url }, member)
