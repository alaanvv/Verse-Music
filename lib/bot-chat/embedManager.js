...
module.exports = bot => {
  ...
  bot.chat.downloadImage = async url => {
    const path_ = process.cwd() + '/img/song_.png'
    const path = process.cwd() + '/img/song.png'

    https.get(url, async res => { res.pipe(fs.createWriteStream(path_)) })
  ...
  bot.chat.generateEmbed = async guildID => {
    ...
    if (player?.queue?.current) {
      ...
      await bot.chat.downloadImage(imageUrl)
        .then(path => { attachment = new AttachmentBuilder(path, { name: 'song.png' }) })
  ...
  bot.chat.updateBotChat = async player => {
    if (bot.settingsDB.get(player.guild, 'botChannel')) {
      ...
      const queueMessage = bot.chat.generateQueueMessage(player.guild)
      const embed = await bot.chat.generateEmbed(player.guild)
