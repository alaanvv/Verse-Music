module.exports = bot => {
  ...
  bot.chat.generateQueueMessage = guildID => {
    ...
    if (queue) {
      ...
      songs.forEach((song, index) => queueMessage.push(`${++index}. ${song.title} ${song.duration ? bot.utils.formatTime(song.duration) : ''} ${queue.current.requester.username ? '| ' + queue.current.requester.username : ''}`))
