module.exports = bot => {
  ...
  bot.chat.generateEmbed = async guildID => {
    ...
    const embed = {
      ...
      description: '[Youtube](https://www.youtube.com/channel/UCowmXUPOZgV7LtgReWHQmBg)ㅤ|ㅤ[Github](https://github.com/alaanvv)ㅤ|ㅤ[Source Code](https://github.com/alaanvv/Music-Bot-Like-Hydra)ㅤ|ㅤ[Commands](https://github.com/alaanvv/Verse-Music/blob/main/commands.md)',
    ...
    if (player?.queue?.current) {
      ...
      let loopMode = 'None'
      if (player.trackRepeat) loopMode = 'Song'
      if (player.queueRepeat) loopMode = 'Queue'
      embed.footer = { text: `Queue length: ${player.queue.length} songsㅤ|ㅤLoop mode: ${loopMode}ㅤ|ㅤAutoplay: ${player.get('autoplay') ? 'On' : 'Off'}` }
    }
